...
interface CreateBaseListModalProps {
  onClose: () => void;
}

const EMOJI_OPTIONS = ["🥛", "🧼", "🍗", "🍞", "🥗", "🍎", "🧃", "🍕", "🥫", "🧊"];

export function CreateBaseListModal({ onClose }: CreateBaseListModalProps) {
  const { addBaseList } = useShoppingLists();
  const [step, setStep] = useState<"name" | "products">("name");
  ...
  const handleSave = () => {
    ...
    toast.success("רשימת הבסיס נוצרה בהצלחה!", {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50" onClick={onClose} dir="rtl">
      ...
      >
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
          ...
          <h2 className="text-lg text-gray-900">צור רשימת בסיס חדשה</h2>
        ...
        {step === "name" ? (
          <div className="px-6 py-6 space-y-6">
            ...
            <button
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-600 text-white rounded-full py-3 transition-colors"
            >
              המשך
            </button>
          </div>
          ...
          <div className="px-6 py-6 space-y-6">
            <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-gray-900">{listName}</h3>
              </div>
              <p className="text-sm text-emerald-700">
                {products.length > 0 ? `${products.length} מוצרים` : "עדיין אין מוצרים"}
              </p>
            </div>
            ...
            {products.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-sm text-gray-600 text-right">מוצרים ברשימה:</h4>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-full py-3 transition-colors"
              >
                חזור
              </button>
              <button
                ...
                className="flex-1 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-300 text-white rounded-full py-3 transition-colors flex items-center justify-center gap-2"
              >
